...
class Texture {
	//========This whole public section wrapper was added to handle destruction=========
public:
	unsigned int id = 0;

	//
	Texture() = default;
	explicit Texture(const std::string& path) { id = loadTexture(path); }
...
//Previously only this function existed the top part is a wrapper for destructing
private:
